...
try:
    ...
    if 'eddyqc' in proc_steps:
        ...
        if not resourceExists or overwrite:
            ...
            eddyOutdir=os.path.join(EDDYQCFOLDER,'EDDY')
            ...
            for dwi in dwifiles:
                ...
                if (numslices % 2) == 1:
                    ...
                    newdwi_name=os.path.basename(dwi).split('_dwi')[0] + '_dwi_even.nii.gz'
                    newdwi=os.path.join(eddyworkdir,newdwi_name)
                    evenslices_command = "fslroi {} {} 0 -1 0 -1 0 {}".format(dwi,newdwi,evenslices).split()
                    print(subprocess.check_output(evenslices_command))
                    dwi=newdwi
                    newrpe_name=os.path.basename(rpe).split('_epi')[0] + '_epi_even.nii.gz'
                    newrpe=os.path.join(eddyworkdir,newrpe_name)
                    dwislices_command = "fslroi {} {} 0 -1 0 -1 0 {}".format(rpe,newrpe,evenslices).split()
                    print(subprocess.check_output(dwislices_command))
                    rpe=newrpe
                else:
                    newdwi_name=os.path.basename(dwi)
                    newdwi=os.path.join(eddyworkdir,newdwi_name)
                    cp_command = "cp {} {}".format(dwi,newdwi).split()
                    print(subprocess.check_output(cp_command))
                    dwi=newdwi

                    newrpe_name=os.path.basename(rpe)
                    newrpe=os.path.join(eddyworkdir,newrpe_name)
                    cp_command = "cp {} {}".format(rpe,newrpe).split()
                    print(subprocess.check_output(cp_command))
                    rpe=newrpe


                with open(bvec, 'r') as f:
                        bvecdata= f.read()
                ...
                B0list=np.where(bvaldataarray == 0)
                B0array=B0list[0]
             
                mergestring=""
                numb0s=len(B0array)
                for B0num in B0array:
                    B0=str(B0num)
                    b0tmp=dwi.split('_dwi')[0] + '_b0_{}.nii.gz'.format(B0.zfill(3))
                    extractb0_command = "fslroi {} {} {} 1".format(dwi, b0tmp,B0).split() 
                    print(subprocess.check_output(extractb0_command))
                    mergestring=mergestring + " " + b0tmp
                
                rpeImage=nib.load(rpe)
                numepis=rpeImage.header['dim'][4]
        
                appa=dwi.split('_dwi')[0] + '_appa.nii.gz'
                acqp=dwi.split('_dwi')[0] + '_acqp.txt'
                index=dwi.split('_dwi')[0] + '_index.txt'
                mergeAppa_command="fslmerge -t {} {} {}".format(appa, mergestring.strip(), rpe).split()
                ...
                dwiPE=dwijson['PhaseEncodingDirection']
                dwiRO=dwijson['TotalReadoutTime']
                peMatrix=['0','0','0']
                invMatrix=['0','0','0']
                if dwiPE == 'i':
                   peMatrix[0] = '1'
                ...
                # create mask in original size
                if numslices > evenslices:
                     b0BrainMaskEven=dwi.split('_dwi')[0] + '_b0hifi_brain_even_mask.nii.gz' 
                     cp_command = "cp {} {}".format(b0BrainMask,b0BrainMaskEven).split()
                     print(subprocess.check_output(cp_command))
                     padmask_command = "fslroi {} {} 0 -1 0 -1 0 {}".format(b0BrainMask, b0BrainMask,numslices).split()
                     print(subprocess.check_output(padmask_command))
                ...
                eddymode='cuda'
                if eddymode=='cuda':
                    eddy_command = "eddy_cuda9.1 --imain={} --mask={} --acqp={} --out={} --index={} --bvecs={} --bvals={} --topup={} --slspec={} --repol --cnr_maps --ol_type=both --niter=8 --fwhm=10,6,4,2,0,0,0,0 --mporder=8 --s2v_niter=8 {} --verbose".format(origdwi,b0BrainMask, acqp,eddyc, index,bvec,bval,topres,slspec,eddy_params).split()
                else:
                    eddy_command = "eddy_openmp --imain={} --mask={} --acqp={} --out={} --index={} --bvecs={} --bvals={} --topup={} --repol --cnr_maps --verbose".format(origdwi,b0BrainMask, acqp,eddyc, index,bvec,bval,topres,eddy_params).split()
                ...
                eddyqc_command = "eddy_quad {} -idx {} -par {} -m {} -b {} -o {}".format(eddyc,index,acqp,b0BrainMask,bval,eddyQuadQCOutdir).split()
